.data
		...
		f10:			.float 10						# Load 10 to multiply units and centen
		inputX: 		.asciiz "Please input a x for taylor sin(x)In range [0,1,2]: "
		number1: 	.asciiz " taylor sin(x) de Orden "	
		answMsg: 	.asciiz   " el resultado es "
		jumpLine:	.asciiz   " \n"	
 .text
 main:
 		jal 			loadUnitsAndHundreds
 		jal			 getNumber
 		jal 			taylorSin				# Returns in $v1
 		j   			END					# End proccess
...
loadUnitsAndHundreds:
 ...
 taylorSin:
 		...
 		li			$s7, 3					# Actual "n" [+1 -3 +5 -7 +9 -12]	# EXPONENTE
 		li			$t3, 0					# Counter
		...
		loopSerie:
			...
			move		$a1 $s7 			# move n
 			...
 			mov.s		$f6 $f3				# move answer to divide
 			mov.s		$f3 $f24			# empty $f3

			jal			floatDivision		# invoke floatDivision

			li			$t4 0				# add or sub it depends of $t4
			beq			$t4 $t3	 minus
			li			$t4 2
			beq			$t4 $t3	 minus
			li			$t4 4
			...
			li			$t4 12
			beq			$t4 $t3	 minus													
			add.s		$f31 $f31 $f30		# add serie
			j			exiit
			minus:			
				sub.s		$f31 $f31 $f30	# sub serie		

			exiit:
				mov.s		$f12 $f31		# move for print
				jal			showAnswer	# print procedure
				mov.s		$f12 $f24		# empty $f12 register
				addi		$s7 $s7 2		# cont = cont + 2
				addi		$t3 $t3 1		# t3 default
				j			loopSerie		# return
...
integerDivision:
		...
		exitLoopInt:	
			jr			$ra 				# return
showAnswer:	
		addi			$t5 $t3 1				# Load the order
		li	 			$v0,  4					# Send the first message
		la	 			$a0, number1
		syscall	
		
		li				$v0, 1					# Print the order
		move			$a0 $t5
		syscall
		
		li	 			$v0,  4					# Print the answer message
		la	 			$a0, answMsg
		syscall	
